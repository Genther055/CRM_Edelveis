  ...
  const [leads, setLeads] = useState<Lead[]>([
    {
      ...
      status: 'new',
      date: '2026-07-24',
      notes: 'Потрібно надрукувати 50 меню А4 на щільному папері з ламінацією.',
    ...
    },
    {
      ...
      status: 'contact',
      date: '2026-07-23',
      notes: 'Дизайн візитки є в наявності. Двостороння матова ламінація.',
      tags: ['Новий'],
      files: [],
            ...
            <tbody>
              {filteredLeads.length === 0 ? (
                ...
                filteredLeads.map(lead => (
                  ...
                  >
                    <td style={{ fontWeight: '600', color: '#475569' }}>{lead.id}</td>
                    <td style={{ fontFamily: 'var(--font-mono)' }}>{lead.date}</td>
                    <td>
                      <div style={{ fontWeight: '700', color: '#0f172a' }}>{lead.name}</div>
                      <div style={{ fontSize: '10px', color: '#64748b', marginTop: '2px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', maxWidth: '280px' }}>
                        {lead.notes}
                      </div>
                    </td>
                    <td>
                      <div>{lead.contactPerson}</div>
                      <div style={{ fontSize: '10px', color: '#64748b', fontFamily: 'var(--font-mono)' }}>{lead.phone}</div>
                    </td>
                    <td>
        ...
        {selectedLead && (
          <div className="ios-card bg-white" style={{ display: 'flex', flexDirection: 'column', gap: '14px', position: 'sticky', top: '20px' }}>
            <div style={{ display: 'flex', justifySelf: 'stretch', justifyContent: 'space-between', alignItems: 'center', borderBottom: '0.5px solid var(--border-light)', paddingBottom: '8px' }}>
              <h3 style={{ fontSize: '13px', fontWeight: '800', color: '#0f172a', textTransform: 'uppercase' }}>Картка запиту</h3>
              <button 
                type="button" 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '12px' }}>
              <div>
                <span style={{ color: '#64748b', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase' }}>Тема / Суть</span>
                <p style={{ fontWeight: '700', fontSize: '13px', color: '#0f172a' }}>{selectedLead.name}</p>
              </div>

              <div>
                <span style={{ color: '#64748b', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase' }}>Контакт</span>
                <p><strong>{selectedLead.contactPerson}</strong></p>
                <p style={{ fontFamily: 'var(--font-mono)', marginTop: '2px' }}>{selectedLead.phone}</p>
                <p style={{ color: '#64748b', fontSize: '11px', marginTop: '1px' }}>{selectedLead.email}</p>
              </div>

              {/* Tags Section */}
              <div>
                <span style={{ color: '#64748b', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase' }}>Теги ліду</span>
              ...
              <div style={{ borderTop: '0.5px solid var(--border-light)', paddingTop: '10px' }}>
                <span style={{ color: '#64748b', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Файли ліду</span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '6px' }}>
                  {(selectedLead.files || []).map(f => (
                    <div key={f} className="flex justify-between items-center bg-[#f9f9f9] p-1.5 rounded" style={{ fontSize: '11px' }}>
              ...
              {/* Custom Fields (with dynamic evaluation of Formulas) */}
              {customFields.length > 0 && (
                <div style={{ borderTop: '0.5px solid var(--border-light)', paddingTop: '10px' }}>
                  <span style={{ color: '#64748b', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Користувацькі поля</span>
                    ...
                    {customFields.map(cf => {
                      ...
                      return (
                        <div key={cf.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
                          <span style={{ color: '#636366' }}>{cf.name}:</span>
              ...
              <div>
                <span style={{ color: '#64748b', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase' }}>Примітки</span>
                <p style={{ backgroundColor: '#f1f5f9', padding: '8px', borderRadius: '4px', fontSize: '11px', marginTop: '2px', whiteSpace: 'pre-wrap' }}>
                  {selectedLead.notes}
                </p>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', borderTop: '0.5px solid var(--border-light)', paddingTop: '10px' }}>
                <div>
                  <span style={{ color: '#64748b', fontSize: '9px', fontWeight: '700', textTransform: 'uppercase' }}>Бюджет</span>
                  <p style={{ fontWeight: '800', fontSize: '14px', color: 'var(--primary)' }}>{selectedLead.budget.toLocaleString()} ₴</p>
                </div>
                <div>
                  <span style={{ color: '#64748b', fontSize: '9px', fontWeight: '700', textTransform: 'uppercase' }}>Статус</span>
